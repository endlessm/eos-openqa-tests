...
sub run {
    ...
    send_key('space');  # select ‘Local Address Book’
    ...
    mouse_set(854, 23);  # ‘Add’ button
    mouse_click('left');

    assert_screen('gnome_contacts_add_contact', 10);

    # Restart gnome-contacts to check that storage is persistent.
    send_key_combo('alt', 'f4');
    check_desktop_clean();

    type_very_safely("gnome-contacts\n");
    assert_screen('gnome_contacts_main_window_one_contact', 10);
